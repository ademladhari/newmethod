...
class MoELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for expert_idx, expert in enumerate(self.experts):
            sample_idx, slot_idx = (topk_indices == expert_idx).nonzero(as_tuple=True)
            if sample_idx.numel() == 0:
                continue
            expert_in = shared_features.index_select(0, sample_idx)
            expert_out = expert(expert_in)
            expert_weights = topk_weights[sample_idx, slot_idx].to(expert_out.dtype).unsqueeze(-1)
            combined.index_add_(0, sample_idx, expert_out * expert_weights)
