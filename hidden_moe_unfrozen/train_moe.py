...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="HiDDeN MoE training (unfrozen branch): warm-start from HiDDeN checkpoint, train encoder + backbone + MoE by default."
    )
    ...
    new_run_parser.add_argument("--num-experts", default=4, type=int, help="Number of experts.")
    new_run_parser.add_argument("--top-k", default=1, type=int, help="Top-k routing.")
    new_run_parser.add_argument("--balance-loss-weight", default=0.04, type=float, help="MoE balance loss weight.")
    ...
    new_run_parser.add_argument("--router-jitter-noise", default=0.0, type=float, help="Router jitter noise.")
    new_run_parser.add_argument("--router-input-dropout", default=0.0, type=float, help="Router input dropout.")
    new_run_parser.add_argument("--router-z-loss-weight", default=0.001, type=float, help="Router z-loss weight.")
    new_run_parser.add_argument("--router-temperature-start", default=1.4, type=float, help="Initial router temperature.")
    ...
    new_run_parser.add_argument("--expert-dropout", default=0.0, type=float, help="Dropout used inside each expert.")
    new_run_parser.add_argument(
        "--expert-weight-decay",
        default=1e-4,
        type=float,
    ...
    new_run_parser.add_argument(
        ...
        action="store_true",
        help="Freeze encoder + decoder.feature_layers (default OFF here: full HiDDeN stack + MoE train).",
    )
    new_run_parser.add_argument(
    ...
    if args.command == "new":
        if hidden_config.freeze_hidden_backbone:
            logging.info("Backbone FROZEN: only MoE router/experts (+ disc if enabled) train.")
        else:
            logging.info(
                "Backbone UNFROZEN: encoder, decoder.feature_layers, MoE router/experts "
                "(+ discriminator if enabled) all train."
            )
    if use_multi_gpu:
